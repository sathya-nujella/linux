...
static int rt711_calibration(struct rt711_priv *rt711)
{
	...
	dev_info(dev, "%s calibration complete\n", __func__);
	return 0;
}
	...
	*mic = false;
	...
	if (jack_status & 0x80) {
		...
		while (loop < 500 &&
			...
			rt711_index_read(rt711->regmap, RT711_VENDOR_REG,
				RT711_COMBO_JACK_AUTO_CTL2, &buf);
		}
...
EXPORT_SYMBOL(rt711_jack_detect);
...
#ifdef CONFIG_PM
static int rt711_suspend(struct snd_soc_component *component)
{
	struct rt711_priv *rt711 = snd_soc_component_get_drvdata(component);

	dev_info(component->dev, "in %s, disabling inits\n", __func__);
...
static int rt711_resume(struct snd_soc_component *component)
{
	dev_info(component->dev, "in %s, nothing to do\n", __func__);

	return 0;
}
#else
#define rt711_suspend NULL
#define rt711_resume  NULL
#endif

static const struct snd_soc_component_driver soc_codec_dev_rt711 = {
	.probe = rt711_probe,
	.set_bias_level = rt711_set_bias_level,
	.suspend	= rt711_suspend,
	.resume		= rt711_resume,
			       ...
			       struct snd_soc_dai *dai)
{
	...
	dev_info(dai->dev, "%s %s", __func__, dai->name);
	stream = snd_soc_dai_get_dma_data(dai, substream);
	...
	case 44100:
		snd_soc_component_write(component, RT711_DAC_FORMAT_H, 0x40);
		snd_soc_component_write(component, RT711_ADC1_FORMAT_H, 0x40);
		snd_soc_component_write(component, RT711_ADC2_FORMAT_H, 0x40);
		break;
	case 48000:
...
int rt711_io_init(struct device *dev, struct sdw_slave *slave)
{
	...
	dev_info(&slave->dev, "%s hw_init complete\n", __func__);
	return ret;
}
