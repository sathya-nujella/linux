...
int rt1308_clock_config(struct device *dev)
{
	...
	pr_info("%s clock_config complete\n", __func__);
...
static int rt1308_read_prop(struct sdw_slave *slave)
{
	...
	prop->source_ports = 0x00;//0x14;	/* BITMAP: 00010100 */
...
#ifdef CONFIG_PM
static int rt1308_sdw_suspend(struct snd_soc_component *component)
{
	struct rt1308_sdw_priv *rt1308 = snd_soc_component_get_drvdata(component);
...
static int rt1308_sdw_resume(struct snd_soc_component *component)
{
...
#else
#define rt1308_sdw_suspend NULL
#define rt1308_sdw_resume  NULL
#endif
...
static const struct snd_soc_component_driver soc_component_sdw_rt1308 = {
	.suspend = rt1308_sdw_suspend,
	.resume = rt1308_sdw_resume,
...
{
	return sdw_register_driver(&rt1308_sdw_driver);
}

static void __exit sdw_slave_exit(void)
{
	sdw_unregister_driver(&rt1308_sdw_driver);
}
